...
void testBounds2(int nb_dims)
{
  ...
  GenoPheno<pwqBoundStrategy, linScalingStrategy> gp(lower_bounds.data(), upper_bounds.data(), nb_dims);
  // Setup parameters
  CMAParameters<GenoPheno<pwqBoundStrategy, linScalingStrategy>> cma_params(nb_dims, init_params.data(), sigma, -1, 0,
                                                                            gp);
...
int main()
{
  for (int i = 1; i < 3; i++)
  {
    std::cout << "==============" << std::endl << "nb_dims: " << i << std::endl;
    testBounds1(i);
    testBounds2(i);
  }
}
